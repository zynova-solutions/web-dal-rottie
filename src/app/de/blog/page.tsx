...
// Example tags/categories for demonstration - translated to German
const tagMap: Record<string, string[]> = {
  'top-5-chicken-dishes': ['Hähnchen', 'Beliebt', 'Würzig'],
  'why-indian-food-is-healthy': ['Gesundheit', 'Ernährung'],
  'spice-meets-simplicity': ['Gewürze', 'Mild', 'Familie'],
  'discover-magic-indian-spices': ['Gewürze', 'Tradition'],
  'what-makes-indian-cuisine-authentic': ['Authentisch', 'Kultur'],
};
...
export default function BlogIndex() {
  ...
  // SEO/OG/LD+JSON
  const canonicalUrl = 'https://dalrotti.com/de/blog';
  ...
  const blogLdJson = {
    ...
    'name': 'Dal Rotti Blog',
    'url': canonicalUrl,
    'description': 'Entdecken Sie Geschichten, Tipps und Neuigkeiten von Dal Rotti',
  ...
  return (
    <>
      <Head>
        <title>Dal Rotti Blog | Authentische Indische Küche Geschichten</title>
        <meta name="description" content="Entdecken Sie Geschichten, Tipps und Neuigkeiten von Dal Rotti" />
        <link rel="canonical" href={canonicalUrl} />
        <meta property="og:title" content="Dal Rotti Blog | Authentische Indische Küche Geschichten" />
        <meta property="og:description" content="Entdecken Sie Geschichten, Tipps und Neuigkeiten von Dal Rotti" />
        <meta property="og:type" content="website" />
        <meta property="og:url" content={canonicalUrl} />
        <meta property="og:image" content={ogImage} />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="Dal Rotti Blog | Authentische Indische Küche Geschichten" />
        <meta name="twitter:description" content="Entdecken Sie Geschichten, Tipps und Neuigkeiten von Dal Rotti" />
        ...
        <div className="hero-content container mx-auto px-4 text-center">
          <div>
            ...
            <p className="text-xl max-w-2xl mx-auto">
              Entdecken Sie Geschichten, Tipps und Neuigkeiten von Dal Rotti
            </p>
          </div>
        </div>
      </section>
        ...
        <ul className="space-y-8">
          {posts.map((post) => (
            <li key={post.slug} className="group border-b pb-6">
              <Link href={`/de/blog/${post.slug}`} className="flex gap-6 items-center rounded-lg transition hover:bg-gray-50 dark:hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-primary p-1 -m-1">
                {post.image && (
                  <Image
                    src={post.image}
                    alt={`Foto für Blogbeitrag: ${post.title}`}
                    width={128}
                    height={96}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-blue-600 hover:underline text-sm font-medium">Weiterlesen →</span>
                </div>
              </Link>
            </li>
          ))}
